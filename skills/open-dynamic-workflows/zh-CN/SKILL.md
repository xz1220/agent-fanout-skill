...
使用流程是：**先复用匹配的受管 workflow → 没有匹配项时才写一段小脚本 →
`odw run` → 检视结果再行动**。任务一次调用就能完成时不要用——直接做。

写任何新脚本前，先运行 `odw workflows list --all`，再用
`odw workflows where <名字>` 核对候选项。优先保留一个可复用的通用 workflow，
不要不断堆积一次性或项目专用 workflow；只有控制流确实不同且可复用时才新建。

面对大型仓库任务，任务数由真实叶子工作决定（文件、组件、测试或可独立核验的证据），
不由适配器数量决定。配置的 concurrency 只是上限：即使上限为 32，只有 6 个大 thunk
也最多产生 6 路并发。应先拆成原子叶子任务，再扇出。要求完整交付时，必须统计 `null`
槽位并恢复或失败，不能把不完整的一批结果静默汇总成成功。
...
| 把 concurrency 配置当成必然达到的并发数 | 它只是上限；要有足够多互相独立的原子 thunk 才能用满。 |
| 期望 `parallel`/`pipeline` 里的失败抛错 | 失败的槽位是 `null`；完整性交付前必须统计并恢复或失败。 |
